...
import com.nxp.nxpwalletconndev.activities.PaymentCardActivity;
...
public final class NfcHttpProxyPersoTask extends AsyncTask<Intent, Void, StringBuffer> {
	private static final String TAG = "NfcHttpProxyPersoTask";
	...
	public NfcHttpProxyPersoTask(Context ctx, String serverUrl, String imei, String scriptIndex, String deviceId, boolean isDefaultProfile, String profile, String pan, String exp) {
		this.ctx = ctx;
		...
		apduListener = (OnTransmitApduListener) ctx;
	}
	...
	protected StringBuffer doInBackground(final Intent... args) {
		...
		try {
			...
			params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterImei, imei));
			params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterId, scriptIndex));
			params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterDevId, deviceId));
			params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterCardData, profile));
			params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterAction, "perso"));
			
			if(isDefaultProfile == false) {
				params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterCardPAN, pan));
				params.add(new BasicNameValuePair(HttpProtokollConstants.ParameterCardExpData, exp));
			}
			...
			while (httpServerResponse.isValid() && httpServerResponse.getType().equalsIgnoreCase(HttpProtokollConstants.TypeValueCommandApdu)
				...
				// Wait some milliseconds to make sure the MCU has time to process
//				Thread.sleep(200);
	...
	protected void onPostExecute(final StringBuffer result) {
		...
		((PaymentCardActivity) ctx).proccessTransactionTaskResult(result);
	}
